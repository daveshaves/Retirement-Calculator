...
int main()
{
	...
	do{
		cout << "Welcome to the retirement calculator!"
			 ...
			 << "\n\t-The annual rate of return on savings will be set to 6% per year"
		...
		int length_retire;
		cin >> length_retire; 
		cout << "Enter desired yearly income: $"; //The amount of income (before inflation) per year you think you'll need in order to fund a comfortable retirement
		double curr_income;
		cin >> curr_income;
		cout << "Enter current savings: $"; //The current value of your savings and investments
		double curr_savings;
		cin >> curr_savings;
		...
		double savings_gap = req_savings - yearly_savings; //Calculates the difference between the total amount you'll have saved and the total amount you'll need to retire
		yearly_savings = yearly_savings_after_inflation(age_diff, future_savings, infl_rate, sav_rate, yearly_savings); //Calls a function that calculates the total value of money put into savings before retirement
		double total_savings = future_savings + yearly_savings; //Calculates the total amount of money you'll have by the time you hit retirement age
		...
		if(total_savings < req_assets){
				double savings_shortfall = req_assets - total_savings;	//If there's a shortfall, this determines the additional amount of money you need to save every year to hit your retirement goal 		
				cout << "\n\tUh oh, looks like you will have a shortfall of $" << savings_shortfall
					 << "\n\tYou will need to put away an extra $" << savings_gap << " per year" << endl;
			}
		else{
